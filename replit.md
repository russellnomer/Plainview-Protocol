...
- **Image Persistence**: Battle cards saved to static/battlecards/ with UUID filenames (PNG format via kaleido)
- **Lightweight Share Handler**: Query parameter routing (?card=<id>) instead of heavy server-side rendering
- **Static File Serving**: Streamlit enableStaticServing for direct image access at /static/battlecards/
- **Rich Twitter Cards**: og:image and twitter:image meta tags (summary_large_image) injected via metadata_handler.py
- **Updated Post to X**: Share URLs use query params for Streamlit compatibility and rich image previews on X/Twitter
